...
export const getSnippetById = async (
  ...
  try {
    const response = await axios.get<CodeSnippet>(
      `${API_BASE_URL}/snippet/snippet-details/${id}/`
    );
    return response.data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      console.error(
        `API Error for snippet ${id}:`,
...
export const filterSnippets = async (
  ...
  return snippets.filter((snippet) => {
    ...
    const matchesDifficulty =
      !difficulty || snippet.difficulty_level === difficulty;
...
export const createSnippet = async (
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error("API Error:", error.response?.data || error.message);
      throw new Error(
        error.response?.data?.detail || "Failed to create snippet"
      );
    } else {
...
export const updateSnippet = async (
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error("API Error:", error.response?.data || error.message);
      throw new Error(
        error.response?.data?.detail || "Failed to update snippet"
      );
    } else {
...
export const getSavedSnippets = async (): Promise<CodeSnippet[]> => {
  try {
    const token = localStorage.getItem("token");
    const response = await axios.get<CodeSnippet[]>(
      `${API_BASE_URL}/snippet/snippet-saved-list/`, // Update the URL to match backend
      {
        headers: {
          Authorization: `JWT ${token}`,
        },
...
export const toggleSaveSnippet = async (
  id: string,
  saved: boolean
): Promise<CodeSnippet> => {
  try {
